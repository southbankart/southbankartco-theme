...
theme.Sections.prototype = _.assignIn({}, theme.Sections.prototype, {
  _createInstance(container, constructor) {
    ...
    const instance = _.assignIn(new constructor(container), {
      id,
      type,
      container,
    });
  ...
  _onSectionUnload(evt) {
    this.instances = _.filter(this.instances, (instance) => {
  ...
  _onSelect(evt) {
    ...
    const instance = _.find(this.instances, (instance) => {
...
theme.Images = (function() {
  ...
  function switchImage(image, element, callback) {
    ...
    const imageUrl = this.getSizedImageUrl(image.src, size);
...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
     ...
     */
    _getCurrentOptions() {
      let currentOptions = _.map($(this.singleOptionSelector, this.$container), (element) => {
    ...
    _getVariantFromOptions() {
      ...
      const found = _.find(variants, (variant) => {
...
window.Drawers = (function() {
  const Drawer = function(id, position, options) {
    const defaults = {
...
theme.Hero = (function() {
  ...
  function Hero(sectionId) {
    ...
    this.$hero.on(`init reInit afterChange${this.namespace}`, this._countSlide.bind(this));
    this.$hero.on(`init reInit afterChange${this.namespace}`, this._showActiveContent.bind(this));
    this.$hero.on(`init reInit afterChange${this.namespace}`, this._showContent.bind(this));
    this.$hero.on(`beforeChange${this.namespace}`, this._hideContent.bind(this));

    const adaptHeight = this.$hero.data('adapt-height');
  ...
  Hero.prototype = _.assignIn({}, Hero.prototype, {
    _setSlideshowHeight() {
      const minAspectRatio = this.$hero.data('min-aspect-ratio');
      this.$hero.height(this.$heroAdapt.width() / minAspectRatio);
    },

    _countSlide(event, slick, currentSlide) {
      // currentSlide is undefined on init,
      // set it to 0 in this case (currentSlide is 0 based)
      const activeSlide = (currentSlide ? currentSlide : 0) + 1;
      const $heroControlsCount = this.$wrapper.find(selectors.heroControlsCount);
      ...
      if (slick.slideCount > 1) {
        ...
        $heroControlsCount.children('.slide-counter').text(`${activeSlide}/${slick.slideCount}`);
      }
    },

    _showActiveContent(event, slick, currentSlide) {
      // currentSlide is undefined on init,
      // set it to 0 in this case (currentSlide is 0 based)
      // eslint-disable-next-line
      var currentSlide = currentSlide || 0;
      const $heroControlsText = this.$wrapper.find(selectors.heroControlsText);
      const $heroTitle = this.$wrapper.find(selectors.heroTitle);

      if (slick.slideCount > 1) {
        let $currentTitle = $heroTitle.filter(`[data-slide-id="${currentSlide + 1}"]`),
          $currentLink = $heroControlsText.filter(`[data-slide-id="${currentSlide + 1}"]`);
    ...
    _togglePause() {
      if (this.$heroPause.hasClass(classes.isPaused)) {
        const labelPause = this.$heroPause.data('label-pause');
        ...
        const labelPlay = this.$heroPause.data('label-play');
    ...
    _a11y(event, obj) {
      const $list = obj.$list;
      const $heroWrapper = $(event.currentTarget).parents(selectors.heroWrapper);
      const autoplay = this.$hero.data('autoplay');
    ...
    _arrowsInit(event, obj) {
      // Slider is initialized. Setup custom arrows
      const count = obj.slideCount;
      const $slider = obj.$slider;
      const $arrows = this.$heroControlsArrow;

      if ($arrows.length && count > 1) {
        $arrows.on(`click${this.namespace}`, (evt) => {
          evt.preventDefault();
          const $arrowButton = $(evt.currentTarget).find('button');
          const arrowButtonControl = $arrowButton.data('control');
    ...
    _scrollTop() {
      ...
      const heroOffset = this.$hero.offset().top;
...
window.Modals = (function() {
  const Modal = function(id, name, options) {
    const defaults = {
  ...
  Modal.prototype.init = function() {
    const $openBtn = $(this.config.open);
...
window.Meganav = (function() {
  const Meganav = function(options) {
    this.cache = {
      $document: $(document),
      $page: $('.page-element'),
    };
  ...
  Meganav.prototype.init = function() {
    const $openBtn = this.config.$meganavToggle;

    $openBtn.on('click', $.proxy(this.requestMeganav, this));

    if (this.config.closeThirdLevelOnBlur) {
      this.config.$meganavLinkThirdLevel.on('blur', $.proxy(this.closeThirdLevelMenu, this));
    }
  };
  ...
  Meganav.prototype.requestMeganav = function(evt) {
    ...
    const anotherNavIsOpen = this.config.isOpen;
    const isThirdLevelBtn = $el.hasClass(this.config.thirdLevelClass);
    ...
    if ($targetedMeganav.hasClass(this.config.drawerClass)) {
      ...
      $el.toggleClass(this.config.activeClass).attr('aria-expanded', !isExpanded);
  ...
  Meganav.prototype.updateThirdLevelContainerWidth = function($el, $dropdown) {
    const $thirdLevel = $dropdown.find(this.config.$meganavDropdownThirdLevel);

    if (!$thirdLevel.length) {
      return;
    }

    $.each($thirdLevel, (key, container) => {
      const $container = $(container);
      const $lastChild = $container.find('li:last-child');
  ...
  Meganav.prototype.updateContainerWidth = function(container, element) {
    ...
    const containerFixedWidth = elementRect.left + columnWidth - containerRect.left;
    const numberOfColumns = containerFixedWidth / columnWidth;
    const containerPercentageWidth = numberOfColumns * 20;
  ...
  Meganav.prototype.close = function(evt, $target) {
    if (this.config.preventDuplicates) {
      ...
      // Close targeted nav
      const $targetedMeganav = $(`#${$target.attr('aria-controls')}`);
  ...
  Meganav.prototype.removeMenuActiveState = function() {
    const activeClasses = [this.config.activeClass, this.config.noAnimationClass];
  ...
  Meganav.prototype.bindEvents = function() {
    ...
    this.config.$meganavs.on('click.meganav', (evt) => {
      // 3rd level container
      const is3rdLevelMenuTarget =
...
window.QtySelector = (function() {
  ...
  QtySelector.prototype.createInputs = function() {
    ...
    const source = $('#QuantityTemplate').html();
  ...
  QtySelector.prototype.validateAvailability = function(line, quantity) {
    const product = theme.cartObject.items[line - 1]; // 0-based index in API
    const handle = product.handle; // needed for the ajax request
    const id = product.id; // needed to find right variant from ajax results
  ...
  QtySelector.prototype.adjustQty = function(evt) {
    const $el = $(evt.currentTarget);
    const $input = $el.siblings('.js-qty__input');
    let qty = this.validateQty($input.val());
    const line = $input.attr('data-line');
    ...
    if (this.settings.isCartTemplate) {
      this.$wrapper.on(
        ...
        $.proxy(function(evt) {
          ...
          const line = $input.attr('data-line');
          const qty = this.validateQty($input.val());
  ...
  QtySelector.prototype.updateCartItemCallback = function(cart) {
    ...
    const items = [];
    let item = {};
    const source = $('#CartProductTemplate').html();
    const template = Handlebars.compile(source);
    let prodImg;
    ...
    $.each(cart.items, (index, cartItem) => {
      ...
      if (cartItem.line_level_discount_allocations.length !== 0) {
        for (const discount in cartItem.line_level_discount_allocations) {
          const amount = cartItem.line_level_discount_allocations[discount].amount;

          cartItem.line_level_discount_allocations[
            discount
          ].formattedAmount = theme.Currency.formatMoney(amount, theme.moneyFormat);
        }
      }
      ...
      if (cartItem.unit_price_measurement) {
        unitPrice = {
          ...
          price: theme.Currency.formatMoney(cartItem.unit_price, theme.moneyFormat),
      ...
      item = {
        ...
        unitPrice,
        linePrice: theme.Currency.formatMoney(cartItem.final_line_price, theme.moneyFormat),
    ...
    this.cache.$cartTable.find('input[type="number"]').each((i, el) => {
      new QtySelector($(el));
    });
    ...
    if (cart.cart_level_discount_applications.length !== 0) {
      for (const cartDiscount in cart.cart_level_discount_applications) {
        ...
        cart.cart_level_discount_applications[
          cartDiscount
        ].formattedAmount = theme.Currency.formatMoney(cartAmount, theme.moneyFormat);
      }
    }

    const source = this.cache.$cartFooterTemplate.html();
    const template = Handlebars.compile(source);
    const data = {
...
window.Notify = (function() {
  ...
  notify.prototype.adaptNotification = function() {
    let adaptHeight = $('.hero').data('adapt-height'),
    ...
    $notification.toggleClass('notification-adapt page-width', adaptHeight === true);
...
theme.Maps = (function() {
  ...
  function Map(container) {
    ...
    const key = this.$container.data('api-key');
  ...
  function geolocate($map) {
    ...
    const address = $map.data('address-setting');
  ...
  Map.prototype = _.assignIn({}, Map.prototype, {
    createMap() {
      const $map = this.$container.find('.map-section__container');
      ...
      return geolocate($map)
        .then((results) => {
          ...
          const map = (this.map = new google.maps.Map($map[0], mapOptions));
          const center = (this.center = map.getCenter());
          const enablePin = $map.data('enable-pin');

          const markerColor = $map.data('marker-color');
        ...
        .fail(() => {
          ...
          if (Shopify.designMode) {
            const $mapContainer = $map.parents('.map-section');

            $mapContainer.addClass('page-width map-section--load-error');
            $mapContainer.find('.map-section__content-wrapper').remove();
            $mapContainer
              .find('.map-section__wrapper')
              .html(`<div class="errors text-center" style="width: 100%;">${errorMessage}</div>`);
          }
        });
    },
...
theme.stickyHeader = (function() {
  ...
  function stickyHeaderOnScroll() {
    const scroll = cache.$window.scrollTop();
    const $el = $(selectors.stickyNavWrapper);
    let threshold = $el.offset().top + $el.height() + 10; // default for scrolling down
...
theme.headerNav = (function() {
  ...
  function sizeNav() {
    let navWidth = 0;
    const parentWidth = $(selectors.siteNavParent).width();
    ...
    $(selectors.siteNavItem).each((i, el) => {
      navWidth += $(el).width();
    });
  ...
  function initHeaderSearch() {
    ...
    $searchForm.each((i, el) => {
      const $form = $(el);
      const $input = $form.find(`.${config.searchInputClass}`);
      const $submit = $form.find(`.${config.searchSubmitClass}`);

      $input.add($submit).on('focus blur', () => {
...
theme.customerTemplates = (function() {
  ...
  function customerAddressForm() {
    ...
    $('.address-delete').on('click', function() {
      const $el = $(this);
      const addressUrl = $el.data('address-url');
...
theme.HeaderSection = (function() {
  ...
  function drawerSearch() {
    const $drawerSearch = $('.drawer__search-input');
    const $drawerSearchSubmit = $('.drawer__search-submit');

    $drawerSearchSubmit.on('click', (evt) => {
...
theme.Product = (function() {
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    ...
    addVariantInfo() {
      ...
      const variantInfo = JSON.parse($(`#VariantJson-${this.settings.sectionId}`).html());
    ...
    _updateStock(variant) {
      ...
      const $stock = $(this.selectors.productStock);
    ...
    _updateVariantChange(evt) {
      ...
      if (variant) {
        ...
        if (variant.unit_price_measurement) {
          const $unitPrice = $(this.selectors.unitPrice, this.$container);
          const $unitPriceBaseUnit = $(this.selectors.unitPriceBaseUnit, this.$container);

          $unitPrice.html(theme.Currency.formatMoney(variant.unit_price, theme.moneyFormat));
          $unitPriceBaseUnit.html(this._getBaseUnit(variant));

          $(this.selectors.unitPriceContainer, this.$container).removeClass('hide');
        }
      } else {
    ...
    setActiveThumbnail(imageId) {
      ...
      const $thumbnails = $(this.selectors.productThumbsWrapper, this.$container);
      ...
      if ($thumbnails.hasClass('slick-initialized')) {
        ...
        const currentActiveSlideIndex = $thumbnails.slick('slickCurrentSlide');
        const newActiveSlideIndex = parseInt($thumbnailToShow.attr('data-slick-index'));
    ...
    _productZoomImage() {
      ...
      this.ProductModal.$modal.on(`click${this.settings.namespace}`, (evt) => {
    ...
    /*
      Thumbnail slider
     */
    _productThumbnailSlider() {
      const $productThumbsWrapper = $(this.selectors.productThumbsWrapper);
      const $productThumbs = $(this.selectors.productThumbs);
      ...
      if ($productThumbs.length > 2) {
        ...
        let nextArrow;
        let prevArrow;
        const sliderArrows = window.sliderArrows || false;

        // sliderArrows is an object defined in product.liquid to set custom
        // SVG arrow icons.
        if (sliderArrows) {
          nextArrow = `<button type="button" class="slick-next"><span class="medium-up--hide">${
            sliderArrows.right
          }</span><span class="small--hide">${sliderArrows.down}</span></button>`;
          prevArrow = `<button type="button" class="slick-prev"><span class="medium-up--hide">${
            sliderArrows.left
          }</span><span class="small--hide">${sliderArrows.up}</span></button>`;
        }
    ...
    _productSwipeInit(evt, obj) {
      ...
      $(this.selectors.productImageWrappers).on('swipeleft swiperight', (event) => {
        ...
        $(`.product-single__thumbnail-item[data-slick-index="${self.settings.productThumbIndex}"]`)
    ...
    _initQtySelector() {
      this.$container.find('.product-form__quantity').each((i, el) => {
        // eslint-disable-next-line no-new
        new QtySelector($(el));
      });
    },
...
theme.Slideshow = (function() {
  ...
  Slideshow.prototype = _.assignIn({}, Slideshow.prototype, {
    ...
    onBlockSelect(evt) {
      const $slide = $(`.hero__slide--${evt.detail.blockId}:not(.slick-cloned)`);
...
theme.Cart = (function() {
  ...
  Cart.prototype = _.assignIn({}, Cart.prototype, {
    ...
    _initQtySelector() {
      $(selectors.cartQtyInput).each((i, el) => {
        // eslint-disable-next-line no-new
        new QtySelector($(el));
      });
    },
    ...
    _initCartNote() {
      ...
      const $el = $(selectors.cartNote);
      let noteText;
      let params;
      const noteOffset = $el[0].offsetHeight - $el[0].clientHeight;
...
theme.Quotes = (function() {
  ...
  Quotes.prototype = _.assignIn({}, Quotes.prototype, {
    _a11y(event, obj) {
      const $list = obj.$list;
      const $wrapper = this.$container.parent();

      // Remove default Slick aria-live attr until slider is focused
      $list.removeAttr('aria-live');

      // When an element in the slider is focused set aria-live
      $wrapper.on('focusin', (evt) => {
    ...
    onBlockSelect(evt) {
      ...
      const $slide = $(`.quote__slide-wrapper--${evt.detail.blockId}:not(.slick-cloned)`);
      const slideIndex = $slide.data('slick-index');
...
theme.Video = (function() {
  ...
  const youtube = {
    promiseAPI() {
      if (!promiseYoutubeAPI) {
        ...
        promiseYoutubeAPI = $.Deferred((defer) => {
    ...
    promisePlayer(id, options) {
      return this.promiseAPI().then(() => {
        return $.Deferred((defer) => {
          ...
          const player = new YT.Player(id, options); // global YT variable injected by YouTube API
  ...
  const vimeo = {
    promiseAPI() {
      if (!promiseVimeoAPI) {
        promiseVimeoAPI = $.Deferred((defer) => {
          const tag = document.createElement('script');
  ...
  Video.prototype = _.assignIn({}, Video.prototype, {
    ...
    _loadPlayer() {
      ...
      const $playerContainer = $(selectors.playerContainer, $container);
      const playerType = this.$container.attr('data-video-type');

      let promiseVideoPlayer;
...
theme.CollectionsList = (function() {
  function CollectionsList(container) {
    const $container = (this.$container = $(container));
    const stretchImage = $container.is('[data-stretch-image]');
    ...
    const namespace = (this.namespace = `.${$container.attr('data-section-id')}`);
    const self = this;
  ...
  CollectionsList.prototype = _.assignIn({}, CollectionsList.prototype, {
    ...
    _collectionListFix() {
      const numberRows = this.$container.find('.grid').data('number-rows');
      const $featureCards = this.$container.find('.featured-card');
      ...
      for (let i = 0; i < numberRows; i++) {
        ...
        const $currentRow = $featureCards.filter(`[data-row-number='${i + 1}']`);
        const $cardHeaders = $currentRow.find('.featured-card__header');

        // Find the max heights for each row
        $currentRow.each(function() {
          const $cardTitle = $(this).find('.featured-card__title');
          const $cardAction = $(this).find('.featured-card__action');
          const $cardImageWrapper = $(this).find('.featured-card__image-wrapper');
          const headerHeight = $cardTitle.outerHeight() + $cardAction.outerHeight() + 65;
          const wrapperHeight = $cardImageWrapper.outerHeight();
...
theme.ProductRecommendations = (function() {
  function ProductRecommendations(container) {
    ...
    const baseUrl = this.$container.data('baseUrl');
    const productId = this.$container.data('productId');
    const recommendationsSectionUrl = `${baseUrl}?section_id=product-recommendations&product_id=${productId}&limit=6`;

    $.get(recommendationsSectionUrl).then((section) => {
      const recommendationsMarkup = $(section).html();
...
theme.articleImages = function() {
  const $indentedRteImages = $('.rte--indented-images');
  if (!$indentedRteImages.length) {
    return;
  }

  $indentedRteImages.find('img').each((i, el) => {
    const $el = $(el);
